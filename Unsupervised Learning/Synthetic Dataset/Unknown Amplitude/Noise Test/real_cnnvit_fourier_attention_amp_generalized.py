...
class CNNDecoder(nn.Module):
    def __init__(self, embed_dim):
        ...
        self.deconv1 = nn.Conv2d(embed_dim, 256, kernel_size=3, padding=1)  # from 128→256

        self.upsample2 = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=False)
        self.deconv2 = nn.Conv2d(256, 128, kernel_size=3, padding=1)        # from 64→128
    ...
    def forward(self, tokens, skip, fixed_support, prior_amp=None):
        ...
        amp = apply_support_mask(amp, fixed_support)

        # L2 (energy) normalisation inside support
        mask = (fixed_support > 0).float()
        prior = prior_amp if (prior_amp is not None) else mask  # <— use soft prior when provided
        eps = 1e-8
        ...
        # Phase (cos/sin raw here; atan2 + mask happens in ViT.forward)
