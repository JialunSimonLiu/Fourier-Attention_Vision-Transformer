...
class CNNDecoder(nn.Module):
    def __init__(self, embed_dim):
        ...
        self.deconv1 = nn.Conv2d(embed_dim, 128, kernel_size=3, padding=1)
        self.relu1 = nn.ReLU()
        ...
        self.upsample3 = nn.Upsample(scale_factor=1, mode='bilinear', align_corners=False)  # just to match skip size
        self.deconv3 = nn.Conv2d(64, 32, kernel_size=3, padding=1)
        self.relu3 = nn.ReLU()

        # 🔹 Feature Fusion: real skip + FFT magnitude of skip
        self.fuse = nn.Sequential(
            nn.Conv2d(32 + embed_dim * 2, 64, kernel_size=3, padding=1),
            nn.ReLU(),
            nn.Conv2d(64, 32, kernel_size=3, padding=1),
            nn.ReLU()
        )

        # 🔹 Residual Phase Refinement
    ...
    def forward(self, tokens, skip):
        B, N, E = tokens.shape
        side = int(N ** 0.5)
        x = tokens.view(B, E, side, side)  # (B, E, H, W)
        ...
        # Resize to skip size
        final_size = skip.shape[-2:]
        x = F.interpolate(x, size=final_size, mode='bilinear', align_corners=False)

        # FFT mag injection
        skip_fft_mag = torch.abs(torch.fft.fft2(skip.to(torch.float32)))
        x = torch.cat([x, skip, skip_fft_mag], dim=1)  # [B, 32 + skip + fft] = [B, 32 + 128 + 128]
        ...
        return x  # final 2-channel output: (cos, sin)
...
class FourierTransformerBlock(nn.Module):
    def __init__(self, embed_dim=64, height=8, width=8, mlp_dim=128, dropout=0.1, use_multiscale=True):
        super().__init__()
        self.norm1 = nn.LayerNorm(embed_dim)
        self.fourier_attn = FlexibleSpectralAttentionMixer(embed_dim, height, width, num_scales=3) if use_multiscale else FourierAttentionWithMask(embed_dim, height, width)
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=64, patch_size=8, in_channels=1, embed_dim=128,
                 num_layers=8, mlp_dim=256, dropout=0.1, use_multiscale=True):
        super().__init__()
    ...
    def forward(self, x, fixed_amplitude):
        ...
        for block in self.transformer:
            x = block(x)
        x = self.norm(x)
        predicted_phase = self.decoder(x, skip)
        ...
        predicted_phase = apply_support_mask(predicted_phase, fixed_amplitude)
