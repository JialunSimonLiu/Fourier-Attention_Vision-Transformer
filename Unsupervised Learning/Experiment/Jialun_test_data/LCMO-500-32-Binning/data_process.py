...
if __name__ == "__main__":
    ...
    amplitude_from_intensity = intensity_to_amplitude("UCL1014a_S0500_frame32-scale.tif")
    ...
    # --- Load phase and support ---
    phase_raw = tiff.imread("phase_old.tif").astype(np.float32)
    support = tiff.imread("support_original.tif").astype(np.float32)

    # Make a clean binary support mask (1 inside, 0 outside)
    support_mask = (support > 0).astype(np.uint16)

    # --- Step 1: Detect scale ---
    # If background is ~314159, data is likely scaled by 1e5
    bg_vals = phase_raw[support_mask == 0]
    if bg_vals.size > 0 and np.median(bg_vals) > 1e4:
        scale_factor = 1e5
    else:
        scale_factor = 1.0
    print(f"Detected scale factor: {scale_factor}")

    # --- Step 2: Convert to radians in [0, 2π) ---
    phase_radians = (phase_raw / scale_factor) % (2 * np.pi)

    # --- Step 3: Rescale to uint16 (0–65535) like phaseA ---
    phase_uint16 = np.uint16((phase_radians / (2 * np.pi)) * 65535.0)

    # --- Step 4: Zero out background using support ---
    phase_uint16 *= support_mask  # background → 0, inside support unchanged

    # --- Step 5: Save cleaned version ---
    tiff.imwrite("phase.tif", phase_uint16)
    print("phase.tif saved with phase in [0, 2π) and background = 0.")
